...
/*---------------------------------------------------------------------------*/
int
CompareStreamPriority(tcp_stream *x, tcp_stream *y) {
		...
		uint32_t saddr, uint16_t sport, uint32_t daddr, uint16_t dport)
{
...
#if DBG_STARVATION
	stream->last_active_ts = mtcp->cur_ts;
#endif

#if ENABLE_NIC_CACHE
	stream->bib = (blk_info_bundle *)MPAllocateChunk(mtcp->bib_pool);
	if (!stream->bib) {
		MPFreeChunk(mtcp->rv_pool, stream->rcvvar);
		MPFreeChunk(mtcp->flow_pool, stream);
	...
	stream->ps = (pkt_shp *)MPAllocateChunk(mtcp->shp_pool);
	if (!stream->ps) {
		MPFreeChunk(mtcp->rv_pool, stream->rcvvar);
		MPFreeChunk(mtcp->flow_pool, stream);
		MPFreeChunk(mtcp->sv_pool, stream->sndvar);
		MPFreeChunk(mtcp->bib_pool, stream->bib);
		pthread_mutex_unlock(&mtcp->ctx->flow_pool_lock);
		return NULL;
	}
#if ENABLE_ECHO
	TAILQ_INIT(&stream->tbeh);
#endif
#endif
	...
	stream->sndvar->nic_rxq = (uint32_t)mtcp->iom->dev_ioctl(mtcp->ctx, stream->sndvar->nif_out,
			GET_RSS, &mtcp->rx_pkt_index) % META_OFFLOAD_NUM_META_CPUS;
#endif
	...
	NIC_CACHE_LOG_STREAM("CREATED NEW TCP STREAM %d: "
			"%u.%u.%u.%u(%d) -> %u.%u.%u.%u(%d) (ISS: %u)\n", stream->id, 
			sa[0], sa[1], sa[2], sa[3], ntohs(stream->sport), 
			da[0], da[1], da[2], da[3], ntohs(stream->dport), 
			stream->sndvar->iss);
	...
	//TRACE_INFO("Create TCP Stream %d\n", stream->id);
#if ENABLE_FAIR_QUEUE && ENABLE_NIC_CACHE
#if 0
	stream->rss = mtcp->iom->dev_ioctl(mtcp->ctx, stream->sndvar->nif_out, 
			GET_RSS, &mtcp->rx_pkt_index);
#endif
#if SHOW_RSS
	fprintf(stderr, "GET_RSS %u mapped value : %u\n"
			"src_addr : %u.%u.%u.%u dst_addr : %u.%u.%u.%u\n"
			"src_port : %u, dst_port : %u\n"
			"-----------------------------------------------------\n",
			stream->rss, stream->rss % DATAPLANE_MAX_CPUS,
			da[0], da[1], da[2], da[3],
			sa[0], sa[1], sa[2], sa[3],
			ntohs(stream->dport), ntohs(stream->sport));
#endif /* SHOW_RSS */
#else
	UNUSED(rss)
#endif /* ENABLE_FAIR_QUEUE && ENABLE_NIC_CACHE */
...
DestroyTCPStream(mtcp_manager_t mtcp, tcp_stream *stream)
{
	...
	//TRACE_INFO("stream:%p, id:%u, close_reason:%u\n", stream, stream->id, stream->close_reason);
	...
	NIC_CACHE_LOG_STREAM("[DESTROYED} port=%u stream %d, "
			"remaining_flows: %u, seq=%u, ack=%u\n",
			ntohs(stream->dport), stream->id, mtcp->flow_cnt,
			stream->snd_nxt, stream->sndvar->snd_una);
/*
	if (g_dbg_port == ntohs(stream->dport))
		TRACE_INFO("Free %u\n", ntohs(stream->dport));*/
...
#if ENABLE_NIC_CACHE
	MPFreeChunk(mtcp->shp_pool, stream->ps);
	MPFreeChunk(mtcp->bib_pool, stream->bib);
#endif
	MPFreeChunk(mtcp->rv_pool, stream->rcvvar);
	MPFreeChunk(mtcp->sv_pool, stream->sndvar);
	...
	//fprintf(stderr, "Destroy_TCP_STREAM\n");

	//TRACE_INFO("Destroyed. Remaining flows: %u\n", mtcp->flow_cnt);
...
DumpStream(mtcp_manager_t mtcp, tcp_stream *stream)
{
	...
	if (sndvar->sndbuf) {
		thread_printf(mtcp, mtcp->log_fp, 
				...
				sndvar->sndbuf->total_buf_size, sndvar->sndbuf->total_buf_size, sndvar->sndbuf->total_buf_size);
	} else {
