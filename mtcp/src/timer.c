...
HandleRTO(mtcp_manager_t mtcp, uint32_t cur_ts, tcp_stream *cur_stream)
{
	...
	TAILQ_HEAD(, flex_buffer) *flex_buffer_list = cur_stream->sndvar->sndbuf->flex_buffer_list;

	TAILQ_FOREACH(flex_buf, flex_buffer_list, flex_buffer_link) {
		fprintf(stderr, "%s,", FLEX_BUFFER_TYPE(flex_buf));
	}
	fprintf(stderr, "prev:%s\n", cur_stream->sndvar->sndbuf->prev_type == 0 ? "FRD" : "FILE_BUFFER");
#endif
	/* Retransmission */
	...
	NIC_CACHE_LOG_RTO("[TIMEOUT] dport=%u, una=%u, seq=%u rto=%u, cur_ts=%u\n", 
			ntohs(cur_stream->dport), cur_stream->sndvar->snd_una, cur_stream->snd_nxt, 
			cur_stream->sndvar->rto, cur_ts);

	TRACE_SINGLE_FLOW(cur_stream->dport, "seq=%u, una=%u, rto=%u, cur_ts=%U\n",
			cur_stream->snd_nxt, cur_stream->sndvar->snd_una, cur_stream->sndvar->rto, cur_ts);

	cur_stream->snd_nxt = cur_stream->sndvar->snd_una;

#if SHOW_NIC_CACHE_STATISTICS
	g_nic_cache_stat[mtcp->ctx->cpu].numRTOs++;
#endif

	INCR_NB_TIMEOUT_RETRANS(mtcp->ctx->cpu);
...
CheckTimewaitExpire(mtcp_manager_t mtcp, uint32_t cur_ts, int thresh)
{
	...
	for (walk = TAILQ_FIRST(&mtcp->timewait_list); walk != NULL; walk = next) {
		if (++cnt > thresh)
			break;
		next = TAILQ_NEXT(walk, sndvar->timer_link);
...
CheckConnectionTimeout(mtcp_manager_t mtcp, uint32_t cur_ts, int thresh)
{
	...
	cnt = 0;
	for (walk = TAILQ_FIRST(&mtcp->timeout_list); walk != NULL; walk = next) {
		if (++cnt > thresh)
			break;
		next = TAILQ_NEXT(walk, sndvar->timeout_link);

		if ((int32_t)(cur_ts - walk->last_active_ts) >= CONFIG.tcp_timeout) {
