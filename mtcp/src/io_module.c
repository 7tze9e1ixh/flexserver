...
SetNetEnv(char *dev_name_list, char *port_stat_list)
{
	...
	//	mpz_t _cpumask;
		...
		mpz_clear(_cpumask);
#endif
#define CONTROL_PLANDE_DEDICATED_LCORE_ID 11
		//sprintf(cpumaskbuf, "0-%d,%d", cpu - 1, 11);
		sprintf(cpumaskbuf, "0-%d", cpu - 1);
		...
		TRACE_INFO("Num of availiable lcores = %u\n", rte_lcore_count());
...
#include <rte_flow.h>
		...
		for (ret = 0; ret < num_devices; ret++) {
			...
			//int s = rte_flow_isolate(ret, 1, NULL);
			//printf("%d\n", s);
		}
