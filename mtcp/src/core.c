...
FlushEpollEvents(mtcp_manager_t mtcp, uint32_t cur_ts)
{
	...
	if (ep->waiting && (usrq->num_events > 0 || 
		...
		TRACE_EPOLL("Broadcasting events. num: %d, cur_ts: %u, prev_ts: %u\n", 
				usrq->num_events, cur_ts, mtcp->ts_last_event);
...
HandleApplicationCalls(mtcp_manager_t mtcp, uint32_t cur_ts)
{
	...
	while ((stream = StreamDequeue(mtcp->closeq))) {
		...
		if (sndvar->sndbuf) {
#if ENABLE_NIC_CACHE
			sndvar->fss = sndvar->sndbuf->head_seq + sndvar->sndbuf->total_buf_size;
#else
...
WritePacketsToChunks(mtcp_manager_t mtcp, uint32_t cur_ts)
{
	...
	/* if (mtcp->g_sender->meta_list_cnt) 
		WriteTCPMetaList(mtcp, mtcp->g_sender, cur_ts, thresh);*/
#endif
/*
	NIC_CACHE_LOG_TCP_SEQ("[TX_Q_SIZE] q_sz=%u\n",
	...
	if (cur_ts == g_dbg_ts) 
		TRACE_SINGLE_FLOW(g_dbg_port, "BURSTY CHANNEL len_of_mbuf:%lu\n", 
				mtcp->iom->dev_ioctl(mtcp->ctx, 0, GET_TX_QUEUE_LEN, NULL));
#endif
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
mtcp_manager_t g_mtcp_manager[MAX_CPUS] = {NULL};
...
RunMainLoop(struct mtcp_thread_context *ctx)
{
	...
	frd_rate_limit_setup(mtcp);

	uint32_t ts_show;

#if ENABLE_NIC_CACHE_FUNC_CALL
	nic_cache_mtcp_master_thread_ready();
#endif

	g_mtcp_manager[mtcp->ctx->cpu] = mtcp;

#if SHOW_PKTLOSS
	uint64_t nb_fast_retrans_prev = 0, nb_timeout_prev = 0, nb_tot_pkts_prev = 0;
#endif
	TRACE_INFO("Run MTCPMainLoop\n");

	ts_show = TIMEVAL_TO_TS(&cur_ts);
#if SHOW_NIC_CACHE_STATISTICS
	if (mtcp->ctx->cpu == 0)
		nic_cache_show_statistics();
#endif

	rate_limit_cache_setup(mtcp);

	foc_setup(mtcp, FOC_MAX_OFFLOADS / CONFIG.num_cores);
	...
	while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) {
		...
		for (rx_inf = 0; rx_inf < CONFIG.eths_num; rx_inf++) {
			...
			SIGNAL_TO_CONTROL_PLANE(mtcp->ctx);
		}
		STAT_COUNT(mtcp->runstat.rounds_rx);

		frd_process_response(mtcp);

		foc_transmit(mtcp, ts);
		//rl_inform_to_nic(mtcp);
		...
		for (tx_inf = 0; tx_inf < CONFIG.eths_num; tx_inf++) {
			if (ctx->cpu == 0)
				CONTROL_PLANE_FLUSH_MESSAGES(ctx, tx_inf);
			nb_pkts += mtcp->iom->send_pkts(ctx, tx_inf);
		}

		if (ctx->cpu == 0 && ts - ts_show >= 1000) {
			ts_show = ts;
#if SHOW_PKTLOSS
			uint64_t diff_fast_retrans = nb_fast_retransmission[ctx->cpu] - nb_fast_retrans_prev;
			uint64_t diff_timeout_retrans = nb_timeout_retransmission[ctx->cpu] - nb_timeout_prev;
			uint64_t diff_tot_sent_pkts = nb_tot_pkts[ctx->cpu] - nb_tot_pkts_prev;

			fprintf(stderr, "----------------------------------------------\n"
					"FAST RETRANSMISSION : %6lu (%6.8f)/s \n"
					"TIMEOUT RETRANSMISSION : %6lu (%6.8lf)/s\n"
					"TOTAL SENT PACKETS : %6lu /s\n"
					"ERROR PACKET RATIO : %6.8lf\n",
					diff_fast_retrans, (double)diff_fast_retrans / diff_tot_sent_pkts,
					diff_timeout_retrans, (double)diff_timeout_retrans / diff_tot_sent_pkts,
					diff_tot_sent_pkts, 
					(double)(diff_timeout_retrans + diff_fast_retrans)/diff_tot_sent_pkts);

			nb_fast_retrans_prev = nb_fast_retransmission[ctx->cpu];
			nb_timeout_prev = nb_timeout_retransmission[ctx->cpu];
			nb_tot_pkts_prev = nb_tot_pkts[ctx->cpu];
#endif
		...
		if (nb_pkts > 0) 
			INCR_PKT_PROC_TIME(GetCurUs() - us_cur);
		...
		INCR_TOT_TIMES(GetCurUs() - us_cur);
	}
...
#if ENABLE_FLOW_SCHED
	sender->send_pq = calloc(1, sizeof(struct send_pq));
	if (!sender->send_pq) {
		return NULL;
	}
	pq = sender->send_pq;
...
InitializeMTCPManager(struct mtcp_thread_context* ctx)
{
...
#if ENABLE_NIC_CACHE
	sprintf(pool_name, "shp_pool_%d", ctx->cpu);
	mtcp->shp_pool = MPCreate(pool_name, sizeof(pkt_shp), 
			sizeof(pkt_shp) * CONFIG.max_concurrency);
	if (!mtcp->shp_pool) {
		CTRACE_ERROR("Fail to allocate pkt shp pool.\n");
		return NULL;
	}

	sprintf(pool_name, "bib_pool_%d", ctx->cpu);
	mtcp->bib_pool = MPCreate(pool_name, sizeof(blk_info_bundle), 
			sizeof(blk_info_bundle) * CONFIG.max_concurrency);
	if (!mtcp->bib_pool) {
		CTRACE_ERROR("Fail to allocate block info bundle pool.\n");
		return NULL;
	}

#if ENABLE_ECHO
	sprintf(pool_name, "to_be_echoed_%d", ctx->cpu);
	mtcp->tbe_pool = MPCreate(pool_name, sizeof(struct to_be_echoed),
			sizeof(struct to_be_echoed) * CONFIG.max_concurrency * NB_TO_BE_ECHOED);
	if (!mtcp->tbe_pool) {
		CTRACE_ERROR("Fail to allocate tbe pool\n");
		return NULL;
	}
#endif
#endif
...
#if ENABLE_META_BATCH
	mtcp->mo = meta_offload_setup(mtcp->ctx->cpu);
	if (!mtcp->mo) {
		CTRACE_ERROR("Failed to create meta_offload resources.\n");
		return NULL;
	}
#endif 
	...
	mtcp->frd_worker = malloc(sizeof(void *) * NUM_FRD_WORKER_PER_CORE);
	if (!mtcp->frd_worker) {
		CTRACE_ERROR("Fail to create frd_worker structure\n");
		return NULL;
	}
	for (i = 0; i < NUM_FRD_WORKER_PER_CORE; i++) {
		mtcp->frd_worker[i] = frd_create_worker(mtcp, mtcp->ctx->cpu, (uint16_t)i);
		if (!mtcp->frd_worker) {
			CTRACE_ERROR("Fail to create frd_worker structure\n");
			return NULL;
		}
...
MTCPRunThread(void *arg)
{
...
#if ENABLE_FLEX_BUFFER
	flex_buffer_pool_create(mtcp, CONFIG.max_concurrency);
	cache_buffer_pool_create(mtcp, CONFIG.max_concurrency);
	general_data_buffer_pool_create(mtcp, CONFIG.sndbuf_size, CONFIG.max_concurrency);
#endif
	...
	//TRACE_INFO(" LOCK STREAM QUEUE %u\n", LOCK_STREAM_QUEUE);
...
#if ENABLE_ASYNC_FILE_READ
	frd_destroy_worker(mtcp);
#endif

#if ENABLE_NIC_CACHE
	MPDestroy(mtcp->shp_pool);
	MPDestroy(mtcp->bib_pool);
#if ENABLE_ECHO
	MPDestroy(mtcp->tbe_pool);
#endif
#endif
...
#if ENABLE_ASYNC_FILE_READ
	frd_global_init(CONFIG.max_concurrency);
#endif

#if ENABLE_NIC_CACHE
	nic_cache_global_init(CONFIG.max_concurrency);
#endif
...
#if DUMP_FLEX_LOG
	flex_log_buf_destroy();
#endif

#if ENABLE_META_OFFLOAD_SEPARATED_CHANNEL
	meta_offload_global_destroy();
#endif
}
/*----------------------------------------------------------------------------*/
