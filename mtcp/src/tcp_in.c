	...
	if (!TCP_SEQ_BETWEEN(seq + payloadlen, cur_stream->rcv_nxt, 
		...
		if (cur_stream->state == TCP_ST_ESTABLISHED) {
			...
			if (cur_stream->state == TCP_ST_TIME_WAIT) {
				...
				TRACE_CLOSE_REASON("AddtoTimewaitList()\n");
		...
		uint16_t window, int payloadlen)
{
...
#if ENABLE_NIC_CACHE	
	if (TCP_SEQ_GT(ack_seq, sndvar->sndbuf->head_seq + sndvar->sndbuf->total_buf_size)) {
#else
	...
	if (TCP_SEQ_LT(ack_seq, cur_stream->snd_nxt)) {
		if (ack_seq == cur_stream->rcvvar->last_ack_seq && payloadlen == 0) {
			if (cur_stream->rcvvar->snd_wl2 + sndvar->peer_wnd == right_wnd_edge) {
				...
				g_nic_cache_stat[mtcp->ctx->cpu].numDUPAcks++;
#endif
	
				}
				dup = TRUE;
				/* SBGetHeadFlexBufferType(sndvar->sndbuf, 
	...
	if (dup && cur_stream->rcvvar->dup_acks == 3) {
...
#if SHOW_NIC_CACHE_STATISTICS
		g_nic_cache_stat[mtcp->ctx->cpu].numFastRetransmissions++;
#endif
		...
		INCR_NB_FAST_RETRANS(mtcp->ctx->cpu);
	...
	NIC_CACHE_LOG_TCP_ACK("[ACK] port=%u, mrtt=%u, srtt=%u, mdev=%u, mdev_max=%u, "
			"cwnd=%u, seq=%u, ack=%u rto=%u, cur_ts=%u\n",
			ntohs(cur_stream->dport), cur_ts - cur_stream->rcvvar->ts_lastack_rcvd, 
			cur_stream->rcvvar->srtt, TS_TO_MSEC((cur_stream->rcvvar->srtt) >> 3), 
			cur_stream->rcvvar->mdev, sndvar->cwnd, 
			seq, ack_seq, sndvar->rto, cur_ts);
	...
	if (rmlen > 0) {
		...
		if (cur_stream->saw_timestamp) {
			...
			NIC_CACHE_LOG_TCP_ACK("[Update RTO] port=%u before_rto=%u, after_rto=%u\n",
					ntohs(cur_stream->dport), sndvar->rto,
...
#if ENABLE_MULTIPATH_SCHED
			if (SBIsHostPath(sndvar->sndbuf)) {
				cur_stream->rcvvar->host_path_rtt += (cur_ts - cur_stream->rcvvar->ts_lastack_rcvd);
				cur_stream->rcvvar->num_host_path++;
#if SHOW_NIC_CACHE_STATISTICS
				g_nic_cache_stat[mtcp->ctx->cpu].host_path_rtt += 
					(cur_ts - cur_stream->rcvvar->ts_lastack_rcvd);
				g_nic_cache_stat[mtcp->ctx->cpu].num_host_path++;
#endif
			} else {
				cur_stream->rcvvar->arm_path_rtt += (cur_ts - cur_stream->rcvvar->ts_lastack_rcvd);
				cur_stream->rcvvar->num_arm_path++;
#if SHOW_NIC_CACHE_STATISTICS
				g_nic_cache_stat[mtcp->ctx->cpu].arm_path_rtt +=
					(cur_ts - cur_stream->rcvvar->ts_lastack_rcvd);
				g_nic_cache_stat[mtcp->ctx->cpu].num_arm_path++;
#endif
			}
#endif
		} else {
...
#if ENABLE_FIXED_CWND
				sndvar->cwnd = MIN(sndvar->cwnd, FIXED_CWND_SIZE);
#endif
...
#if ENABLE_NIC_CACHE
		ret = SBRemove(mtcp->rbm_snd, sndvar->sndbuf, rmlen, mtcp->ctx->cpu, cur_stream->id);
#else
...
// #endif
	...
	TRACE_SINGLE_FLOW(cur_stream->dport, "mrtt=%u, cwnd=%u, seq=%u, ack=%u, "
			"rto=%u, cur_ts=%u payloadlen=%u\n",
			cur_ts - cur_stream->rcvvar->ts_lastack_rcvd, sndvar->cwnd,
			seq, ack_seq, sndvar->rto, cur_ts, payloadlen);
		...
		uint8_t *payload, int payloadlen, uint16_t window)
{
	...
	if (tcph->fin) {
		...
		if (seq + payloadlen == cur_stream->rcv_nxt) {
			...
			TRACE_CLOSE_REASON("stream%p, AddtoTimewaitList()\n", cur_stream);
			AddtoTimewaitList(mtcp, cur_stream, cur_ts);
			AddtoControlList(mtcp, cur_stream, cur_ts);
		}
#if 0
		 ...
		 uint32_t cur_ts, const int ifidx, const struct iphdr *iph, int ip_len)
{
	...
	/*TRACE_INFO("(CPU%d) saddr:%u, daddr:%u, sport:%u, dport:%u\n",
...
int
ProcessEchoPacket(mtcp_manager_t mtcp, uint32_t cur_ts, struct iphdr *iph)
{
	struct tcphdr *tcph = (struct tcphdr *)((uint8_t *)iph + (iph->ihl << 2));
	struct echo_hdr *eh = (struct echo_hdr *)((uint8_t *)tcph + (tcph->doff << 2));
	struct to_be_echoed *tbe;
	...
	tbe = TAILQ_FIRST(&cur_stream->tbeh);
	TAILQ_REMOVE(&cur_stream->tbeh, tbe, tbe_link);
	if (tbe->e_seq != eh->e_seq || tbe->e_len != eh->e_len) {
		MPFreeChunk(mtcp->tbe_pool, tbe);
		TRACE_ERROR("Fail to get echo packets from DPU sent_seq=%u, received_seq=%u\n",
				tbe->e_seq, eh->e_seq);
		return ERROR;
	}
	NIC_CACHE_LOG_ECHO_PKT("[ECHO] dport=%u, Receive e_seq=%u, e_len=%u\n",
			ntohs(s_stream.dport), tbe->e_seq, tbe->e_len);
	/*
	TRACE_INFO("[ECHO] dport=%u, Receive e_seq=%u, e_len=%u, dpu_rtt=%u(ms)\n",
			ntohs(s_stream.dport), tbe->e_seq, tbe->e_len, cur_ts - tbe->sent_ts);*/

#if DEBUG_DPU_RTT
	nic_cache_dump_dpu_rtt(cur_ts - tbe->sent_ts);
#endif
