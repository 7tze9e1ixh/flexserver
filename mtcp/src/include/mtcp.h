...
#define TCP_OPT_TIMESTAMP_ENABLED       FALSE //TRUE   // enabled for rtt measure
#define TCP_OPT_SACK_ENABLED            FALSE //TRUE   // only recv-side implemented

/* increasing initial congestion window size to 10 MSS (rfc6928) */
#define INIT_CWND_10					TRUE
...
#define ENABLE_FLOW_SCHED				FALSE
#define ENABLE_META_CHANNEL				TRUE
#define ENABLE_META_TX_QUEUE			FALSE
#define ENABLE_APP_HDR_BATCH			FALSE
#define ENABLE_MULTI_VFS				FALSE
#define ENABLE_MULTIPATH_SCHED			TRUE

#if ENABLE_META_TX_QUEUE
#define GET_META_TX_QID(cpu)			(CONFIG.num_cores + (cpu))
#endif
...
/*----------------------------------------------------------------------------*/
#define PKT_MTU 1500
#define MAX_TSO_PKT_SIZE 0xffff
#define JUMBO_MTU 9000
#define JUMBO_WITHOUT_HEADER_LEN (JUMBO_MTU - 40)
//#define MAX_TSO_PKT_SIZE (7 * JUMBO_WITHOUT_HEADER_LEN)
/*----------------------------------------------------------------------------*/
/* Statistics */
#ifdef NETSTAT
...
#define SBUF_TRYLOCK(lock)		pthread_spin_trylock(lock)
#else
#define SBUF_LOCK_INIT(lock, errmsg, action);		\
...
#define SBUF_TRYLOCK(lock)		pthread_mutex_trylock(lock)
#endif /* USE_SPIN_LOCK */
...
struct mtcp_sender
{
	...
	TAILQ_HEAD(meta_head, tcp_stream) meta_list;
	int meta_list_cnt;
#endif
...
#if ENABLE_FILE_READ_WORKER || ENABLE_ASYNC_FILE_READ
	void *frd_worker;
#endif
	uint64_t tx_bytes;
	uint64_t tx_limits;
