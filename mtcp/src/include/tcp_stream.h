...
#define MAX_SACK_ENTRY 8
	...
	uint32_t host_path_rtt;
	uint32_t arm_path_rtt;
...
struct tcp_send_vars
{
	...
	TAILQ_ENTRY(tcp_stream) meta_link;
#endif
...
enum tcp_stream_send_prio {
	TCP_STREAM_SEND_PRIO_META = 0,
	TCP_STREAM_SEND_PRIO_NORMAL = 1,
};
#endif
...
typedef struct tcp_stream
{
	...
	uint32_t foc_hv;
	uint8_t foc_state;
	ssize_t foc_file_length;
	char path_name[256];
	TAILQ_ENTRY(tcp_stream) foc_ht_link;
	TAILQ_ENTRY(tcp_stream) foc_wait_link;
#endif

	uint8_t submitToUring;

#if DBG_STARVATION
	uint32_t last_sent_ts;
	uint8_t starved_reason; 
#endif

#if ENABLE_FLOW_SCHED
	uint8_t prio;
	RB_ENTRY(tcp_stream) tree_entry;
#endif
...
#if ENABLE_FLOW_SCHED
RB_HEAD(send_pq, tcp_stream);
RB_GENERATE_INTERNAL(send_pq, tcp_stream, tree_entry, CompareStreamPriority, inline static);
#endif
