...
#define HOST_SEND_PAYLOAD -2

int
mtcp_writev(mctx_t mctx, int sockid, const struct iovec *iov, int numIOV);

ssize_t custom_sendfile(mctx_t mctx, int sockid, int in_fd, off_t *offset, size_t count);

int nic_cache_send(mctx_t mctx, int sockid, char *url, size_t url_len);
