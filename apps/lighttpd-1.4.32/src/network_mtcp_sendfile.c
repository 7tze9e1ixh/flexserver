...
int network_write_chunkqueue_mtcp_sendfile(server *srv, connection *con, int fd, chunkqueue *cq, off_t max_bytes) {
	chunk *c;
	...
	for(c = cq->first; (max_bytes > 0) && (NULL != c); c = c->next) {
		...
		case FILE_CHUNK: {
            ...
            r = nic_cache_send(srv->mctx, fd, c->file.name->ptr, c->file.name->used - 1);
            if (r == 0 || r == -1) {
                switch (errno) {
                case EAGAIN:
                case EINTR:
...
host_send :
			...
			if (-1 == c->file.fd) { // O_SYNC | O_DIRECT | O_RDONLY
				if (-1 == (c->file.fd = open(c->file.name->ptr, O_SYNC | O_DIRECT | O_RDONLY))) {
			...
			if (-1 == (r = custom_sendfile(srv->mctx, fd, c->file.fd, &offset, toSend))) {
				switch (errno) {
				case EAGAIN:
				case EINTR:
...
#define READ_AHEAD 4 M
			...
			if (c->offset == c->file.length) {
				...
				if (c->file.fd != -1) {
					//fprintf(stderr, "close file(fd=%d)\n", c->file.fd);
					/* mTCP stack takes in charge of close() */
					//close(c->file.fd); 
					c->file.fd = -1;
				}
			}
