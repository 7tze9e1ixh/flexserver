...
#define MAX_CPUS 24
unsigned long long g_reqs[MAX_CPUS] = {0};
unsigned long long g_hits[MAX_CPUS] = {0};
unsigned long long g_tx_bytes[MAX_CPUS] = {0};
unsigned char ready[MAX_CPUS] = {0};
static unsigned char g_nb_threads;
static unsigned char run_pst = 0;

inline static double
GetHitRatio(unsigned long long hits_prev[], unsigned long long reqs_prev[])  {

	int i;
	unsigned long long sum_hits = 0, sum_reqs = 0, sum_hits_prev = 0, sum_reqs_prev = 0;
	double hr;

	for (i = 0; i < g_nb_threads; i++) {
		sum_hits += g_hits[i];
		sum_reqs += g_reqs[i];
		sum_hits_prev += hits_prev[i];
		sum_reqs_prev += reqs_prev[i];

		hits_prev[i] = g_hits[i];
		reqs_prev[i] = g_reqs[i];
	}

	hr = (double)(sum_hits - sum_hits_prev) / (sum_reqs - sum_reqs_prev);

	return hr;
}

inline static double
GetTotalTXGbps(unsigned long long tx_bytes_prev[]){

	int i;
	unsigned long long sum_tot_bytes = 0, sum_tot_bytes_prev = 0;
	for (i = 0; i < g_nb_threads; i++) {
		sum_tot_bytes += g_tx_bytes[i];
		sum_tot_bytes_prev = tx_bytes_prev[i];
		g_tx_bytes[i] = tx_bytes_prev[i];
	}

	return (double)(sum_tot_bytes - sum_tot_bytes_prev) * 8 / (1 << 30); 
}

static void *
print_stat_thread(void *arg) 
{
	FILE *sffp;
	int i;
	double goodput, hitratio;
	struct timespec ts;
	unsigned long long  hits_prev[MAX_CPUS] = {0}, 
						reqs_prev[MAX_CPUS] = {0},
						tx_bytes_prev[MAX_CPUS] = {0};
	
	pthread_mutex_t psmtx;
	pthread_cond_t pscnd;

	sffp = fopen("log/status.dat", "w");
	if (!sffp) {
		perror("Fail to open status.dat file");
		exit(EXIT_FAILURE);
	}

	if (pthread_mutex_init(&psmtx, NULL) < 0) {
		perror("Fail to initialize mutex!");
		exit(EXIT_FAILURE);
	}

	if (pthread_cond_init(&pscnd, NULL) < 0) {
		perror("Fail to initialize cond!");
		exit(EXIT_FAILURE);
	}
	...
	/* Wait before all threads are ready */
	while(1) {
		...
		if (count == g_nb_threads)
			break;
	}

	run_pst = 1;
	fprintf(sffp, "%10s  %10s\n", "Goodput(gbps)", "Hit Ratio");
	...
	while (run_pst)
	{
		...
		pthread_cond_timedwait(&pscnd, &psmtx, &ts);

		goodput = GetTotalTXGbps(tx_bytes_prev);
		hitratio = GetHitRatio(hits_prev, reqs_prev);
#if PRINT_STDOUT
		fprintf(stdout, "Goodput(gbps) = %10.2f\t Hit Ratio = %10.2f\n", goodput, hitratio);
#endif
		fprintf(sffp, "%10.2f, %10.2f\n", goodput, hitratio);
		clock_gettime(CLOCK_REALTIME_COARSE, NULL);
		ts.tv_sec++;
		pthread_mutex_unlock(&psmtx);
	}

	pthread_mutex_destroy(&psmtx);
	pthread_cond_destroy(&pscnd);
	fclose(sffp);
...
/*----------------------------------------------------------------------------*/
static void
start_printing_status(void)
{
	/* Print goodput */
	int ret;
	pthread_t pst;

	ret = pthread_create(&pst, NULL, print_stat_thread, NULL);
	if (ret < 0) {
		perror("Fail to create print_stat_thread");
		exit(EXIT_FAILURE);
	}
}
/*----------------------------------------------------------------------------*/
static void
stop_printing_status(void)
{
	run_pst = 0;
}
#endif
